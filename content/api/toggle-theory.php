...
try {
    ...
    $stmt = $conn->prepare("SELECT is_theory, title FROM lessons WHERE id = ?");
    ...
    // تبديل الحالة
    $newState = !$lesson['is_theory'];
    
    // تحديث قاعدة البيانات
    $stmt = $conn->prepare("UPDATE lessons SET is_theory = ?, updated_at = NOW() WHERE id = ?");
    $success = $stmt->execute([$newState, $lessonId]);
    
    if ($success) {
        // تحضير رسالة مخصصة تتضمن عنوان الدرس
        $message = $newState ? 
            sprintf('تم تحديد الدرس "%s" كدرس نظري', $lesson['title']) :
            sprintf('تم إلغاء تحديد الدرس "%s" كدرس نظري', $lesson['title']);
